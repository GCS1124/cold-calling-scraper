...
that exclude lead payloads and raw credentials, and an OpenAPI 3.1 document
served through capabilities. Before production
...
| Trust foundation | Local regression and browser checks pass | 299 server tests, 56 client tests, 9 shared tests, 5 SDK tests, both builds, runtime boot, lint, and mocked browser flow |
...
| Versioned integration surface | Preview: `/api/v1` capabilities, owner-required route aliases, hashed API-key authentication, environment and Postgres key stores, expiry/revocation state, per-credential rate limiting, typed SDK, OpenAPI 3.1 discovery, bounded audit events with required-mode readiness gate, lifecycle/idempotency/error contract, and integration documentation are implemented | Customer-specific quotas, scheduled retention enforcement, worker callbacks, production key operations, and live client interoperability |
