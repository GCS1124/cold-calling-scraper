...
Current implementation records a deterministic website identity assessment, robots outcome, exact assessed page, observation timestamp, and SHA-256 document hash. Parked/challenge pages cannot contribute website contact evidence, robots-disallowed roots are not fetched, and weak contact-bearing pages remain usable but are labeled probable with explicit gaps. Owner feedback suppression is branch-aware: wrong-phone events bind the number to its observed organization/person, while business corrections use address/location identity instead of a bare domain or shared switchboard. The remaining validation is a bounded live-provider/domain smoke matrix and retention-aware reuse of unchanged documents.
...
AI public-profile lens: expand public professional discovery through company leadership pages, associations, and public search references; corroborate current company and role. Reuse a proven company contact route for multiple relevant people while labeling it a business route. Quarantine former/conflicting employment for review.

AI: compile the brief into typed supported criteria; show which requirements are enforced versus still unverified. Route company/people discovery according to intent. Fuse GMB, public professional profiles, Gemini grounding, public websites, and published social links. Honor exclusions. Recover with deterministic query expansion if Gemini is unavailable. Do not label an unsupported criterion as matched. Both public modes expose the same provider-coverage states (`configured`, `not_configured`, `returned`, `failed`, and `partial`) so a zero result cannot be mistaken for an unattempted source.
...
The normalized research path now persists source-family/authority metadata, public profile identity, organization identity, role, employment relationship status, contact observation dates, and idempotent phone/email verification events. Search responses and evidence dossiers now carry lifecycle metadata plus the same phone-qualified quality rollup, including eligible/review counts, freshness, tier distribution, and source-family counts; exports preserve public role and organization fields. Durable search jobs now carry an optional authenticated owner and enforce owner-scoped reads and mutations when auth is enabled. Owner-scoped feedback events and suppression enforcement now cover wrong phone, wrong business, wrong person, former employee, duplicate, do-not-contact, and useful signals across all three response paths. Shared-workspace authorization, reviewed correction policy, and live export checks remain pending.
...
sharing a browser session, start one of the two public modes, safely retry a lost
...
| Trust foundation | Local regression and browser checks pass | 301 server tests, 56 client tests, 9 shared tests, 5 SDK tests, both builds, runtime boot, lint, and mocked browser flow |
...
Owner-scoped lead feedback is now available through the existing internal application transport. The server derives bounded suppression keys from the stored qualified lead, so the browser cannot submit or rewrite a phone number, email, or profile URL. Suppression is applied after the mandatory public-phone gate and before quality summaries, dossiers, and exports; `meta.progress.suppressedCount` and a provider notice make intentional underfill visible. Wrong-phone suppression is associated with the observed organization/person, and business suppression uses address/location-aware identity so a shared domain or switchboard does not erase another branch. Durable feedback requires an authenticated owner and Postgres. Local development may fall back to process memory; Vercel fails closed when durable feedback storage is unavailable. Shared workspace roles, expiry controls, and independently reviewed correction labels remain future work.
